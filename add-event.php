...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $event_time = date("h:i A", strtotime($_POST['event_time'])); // Format the start time to h:i A
    $event_end_time = date("h:i A", strtotime($_POST['event_end_time'])); // Format the end time to h:i Ae
...
<div class="container mt-5" style="margin-top:4rem !important; margin-bottom:4rem !important; max-width: 500px !important; border: 3px solid blue;border-radius:15px">
    ...
    <form action="add-event.php" method="post" style="margin-top:2rem !important; margin-bottom:2rem !important;">
        ...
        <!-- Event Time Dropdown -->
        <!-- Event Start Time Dropdown -->
<div class="form-group">
    <label for="event_time">Event Start Time</label>
    <select class="form-control" id="event_time" name="event_time" required>
        <option value="">Select Start Time</option>
        <?php
        // Loop for generating times from 12:00 AM to 11:30 PM in 12-hour format with AM/PM
        $times = ['AM', 'PM']; // Array for AM/PM
        foreach ($times as $period) {
            for ($hour = 1; $hour <= 12; $hour++) {
                $hour_formatted = str_pad($hour, 2, '0', STR_PAD_LEFT);
                echo "<option value='$hour_formatted:00 $period'>$hour_formatted:00 $period</option>";
                echo "<option value='$hour_formatted:30 $period'>$hour_formatted:30 $period</option>";
            }
        }
        ?>
    </select>
    ...
    for (var periodIndex = periods.indexOf(startPeriod); periodIndex < periods.length; periodIndex++) {
        ...
        for (var hour = (periodIndex == periods.indexOf(startPeriod)) ? startHour + 1 : 1; hour <= 12; hour++) {
            ...
            if (hour > startHour || period != startPeriod) {
                endTimeDropdown.append('<option value="' + hourFormatted + ':00 ' + period + '">' + hourFormatted + ':00 ' + period + '</option>');
                endTimeDropdown.append('<option value="' + hourFormatted + ':30 ' + period + '">' + hourFormatted + ':30 ' + period + '</option>');
            }
        }
    }
});
